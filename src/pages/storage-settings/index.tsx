...
const DEFAULT_LATENCY: Record<string, number> = {
  ...
  'breeze-tts-2': 20,
  // yue2 实测(A100 单卡,YuE2-Turbo 引擎):一个实例同时跑 4 首,满载时 4 首一波 40-47s,
  // 平均每首占卡约 11s(单首 40-80s 的歌 11-25s,翻唱另加 4-30s 扒谱),取 15。准入按
  // 「每实例一次一个任务」估排队,配 15 时 music 类默认 90s 上限在第 7 个排队请求就拒成 429,
  // 而引擎自己的队列有 12 格,要用满就把同页的按模型排队上限一起抬到 ~240。
  yue2: 15
};
