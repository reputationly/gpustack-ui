...
const DEFAULT_LATENCY: Record<string, number> = {
  ...
  'breeze-tts-2': 20,
  // yue2 实测(A100 单卡):歌词决定整首歌的长度,短歌词也会编成 1-2.5 分钟,
  // RTF≈0.45,一首 40-105s(翻唱另加 5-30s 扒谱),取 75。music 类按类别排队上限默认 90s,配 75 的话
  // 每实例第 3 个并发就被准入拒成 429;引擎自己的队列是 8,要放开并发就把同页的
  // 按模型排队上限一起抬到 ~600。
  yue2: 75
};
